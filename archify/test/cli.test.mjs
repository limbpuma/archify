...
test('cli: help lists commands and diagram types', () => {
  ...
  assert.match(result.stdout, /architecture, workflow, sequence, dataflow, lifecycle, flowchart, struktogramm, uml-class, erd, usecase, netzplan/);
});
...
test('cli: guide lists all scenario recipes by diagram type', () => {
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /Archify scenario recipes \(18\)/);
