...
test('generated proof gallery matches its sources, receipts, and checked-in artifacts', () => {
  ...
  assert.match(output, /gallery 20 artifacts \/ \d+ checks/);
  ...
  assert.equal(manifest.entryCount, 20);
  assert.deepEqual(new Set(manifest.entries.map((entry) => entry.type)), new Set([
    'architecture', 'workflow', 'sequence', 'dataflow', 'lifecycle', 'flowchart', 'struktogramm', 'uml-class', 'erd', 'usecase', 'netzplan', 'activity', 'epk',
  ]));
  assert.deepEqual(
  ...
  assert.match(html, /Thirteen lenses\. Twenty real stories\./);
  ...
  assert.equal((html.match(/class="card-link create-link"/g) || []).length, 20);
