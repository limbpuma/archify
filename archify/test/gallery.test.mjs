...
test('generated proof gallery matches its sources, receipts, and checked-in artifacts', () => {
  ...
  assert.deepEqual(new Set(manifest.entries.map((entry) => entry.type)), new Set([
    'architecture', 'workflow', 'sequence', 'dataflow', 'lifecycle', 'flowchart', 'struktogramm', 'usecase',
  ]));
  assert.deepEqual(
    Object.fromEntries(['architecture', 'workflow', 'sequence', 'dataflow', 'lifecycle', 'flowchart', 'struktogramm', 'usecase'].map((type) => [
      type,
      manifest.entries.filter((entry) => entry.type === type).length,
    ])),
    { architecture: 2, workflow: 3, sequence: 2, dataflow: 2, lifecycle: 2, flowchart: 1, struktogramm: 1, usecase: 1 },
  );
  assert.deepEqual(
  ...
  assert.match(html, /Seven lenses\. Fourteen real stories\.|Six lenses\. Thirteen real stories\.|Five lenses\. Twelve real stories\./);
