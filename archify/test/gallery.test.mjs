...
test('generated proof gallery matches its sources, receipts, and checked-in artifacts', () => {
  ...
  assert.match(output, /gallery 17 artifacts \/ \d+ checks/);
  ...
  assert.equal(manifest.entryCount, 17);
  assert.deepEqual(new Set(manifest.entries.map((entry) => entry.type)), new Set([
    'architecture', 'workflow', 'sequence', 'dataflow', 'lifecycle', 'flowchart', 'struktogramm', 'uml-class', 'erd', 'usecase',
  ]));
  assert.deepEqual(
    Object.fromEntries(['architecture', 'workflow', 'sequence', 'dataflow', 'lifecycle', 'flowchart', 'struktogramm', 'erd', 'usecase'].map((type) => [
      type,
      manifest.entries.filter((entry) => entry.type === type).length,
    ])),
    { architecture: 2, workflow: 3, sequence: 2, dataflow: 2, lifecycle: 2, flowchart: 1, struktogramm: 1, erd: 2, usecase: 1 },
  );
  assert.deepEqual(
  ...
  assert.equal((html.match(/class="showcase-card/g) || []).length, 17);
  ...
  assert.match(html, /Ten lenses\. Seventeen real stories\./);
  ...
  assert.equal((html.match(/class="card-link create-link"/g) || []).length, 17);
  for (const type of ['architecture', 'workflow', 'sequence', 'dataflow', 'lifecycle', 'flowchart', 'struktogramm', 'uml-class', 'erd', 'usecase']) {
