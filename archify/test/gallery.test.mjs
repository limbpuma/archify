...
test('generated proof gallery matches its sources, receipts, and checked-in artifacts', () => {
  ...
  assert.deepEqual(
    Object.fromEntries(['architecture', 'workflow', 'sequence', 'dataflow', 'lifecycle', 'flowchart', 'struktogramm', 'uml-class', 'erd', 'epk'].map((type) => [
      type,
      manifest.entries.filter((entry) => entry.type === type).length,
    ])),
    { architecture: 2, workflow: 3, sequence: 2, dataflow: 2, lifecycle: 2, flowchart: 1, struktogramm: 1, 'uml-class': 1, erd: 2, epk: 1 },
  );
  assert.deepEqual(
  ...
  assert.match(html, /Eight lenses\. Sixteen real stories\./);
  ...
  for (const type of ['architecture', 'workflow', 'sequence', 'dataflow', 'lifecycle', 'flowchart', 'struktogramm', 'uml-class', 'erd', 'epk']) {
