---
name: archify
description: Create polished, validated architecture, workflow, sequence, data-flow, lifecycle/state, DIN 66001 flowchart, Struktogramm (DIN 66261), UML class (uml-class), ER (erd, Chen or crow's foot), use case (usecase), activity (activity), network plan (netzplan, DIN 69900), and event-driven process chain (epk) diagrams as explorable standalone HTML with dark/light themes, trace motion, and PNG/SVG/WebM export. Accept plain-language requirements or pasted Mermaid flowchart, sequenceDiagram, stateDiagram, classDiagram, erDiagram, and gantt input. Use when the user asks to visualize architecture, technical workflows, API calls, request lifecycles, data pipelines, state machines, algorithms (Programmablaufplan / PAP), structured programs (Nassi-Shneiderman, IHK-style algorithm descriptions), UML domain models, ER schemas, requirements (actors, system boundary, include/extend), processes with swimlanes and fork/join, project schedules and critical paths (FAZ/FEZ/SAZ/SEZ), business processes (eEPK), or to convert Mermaid.
license: MIT
metadata:
  version: "2.17"
  author: tt-a1i
...
1. Choose `architecture`, `workflow`, `sequence`, `dataflow`, `lifecycle`, `flowchart`, `struktogramm`, `uml-class`, `erd`, `usecase`, `activity`, `netzplan`, or `epk` from the question.
...
Activity note: nodes sit on an explicit `col`/`row` grid (`meta.grid`, default 230×72 px with origin (140, 100)). Lanes are vertical swimlanes defined by `meta.lanes[].cols: [start, end]` (inclusive column ranges), each node must sit inside exactly one lane and below the lane header (`y ≥ 70`), and without lanes the canvas is one partition. Exactly one `initial` node and at least one `final` node, each `decision` has ≥ 2 outgoing edges and every outgoing decision edge carries a guard `label` (the renderer auto-brackets it as `[label]`). `fork` has 1 incoming + ≥ 2 outgoing edges; `join` has ≥ 2 incoming + 1 outgoing edge; `merge` joins ≥ 2 incoming into 1 outgoing. `fork`/`join` are bars (default horizontal 140×6; `orientation: "vertical"` swaps to 6×140). With automatic sides, the continuation of a decision leaves the bottom vertex and a branch leaves the side facing its target. Keep `viewBox[0] ≤ 1380` so the 9 px member text stays ≥ 6 px at the 1440 px desktop projection. Symbols: `initial` / `final` (filled circle, bull's-eye), `action` (rounded rectangle, optional `sublabel`), `decision` / `merge` (diamond, the question sits beside the diamond), `fork` / `join` (bar), `object` (object flow rectangle). Contract and task list: [`renderers/activity/README.md`](renderers/activity/README.md).
...
UML-class note: author a `classes[]` collection on an explicit `meta.grid` (`colWidth` × `rowHeight` in `meta.grid`, default 228 × 160 px, `originX` 116, `originY` 100) and a `relations[]` collection of UML 2.5 lines. Each classifier is a three-compartment box: header (`name`, optionally a stereotype over it), `attributes[]`, and `methods[]` — enums drop the operation compartment unless methods are given. Marker ends always sit at the `to` end of the line: inheritance (solid + hollow triangle), realization (dashed + hollow triangle, must point at an interface), aggregation (hollow diamond at the whole), composition (filled diamond at the whole), dependency (dashed + open arrow), and `association` (open arrow at `to` only when `navigable: true`). Multiplicity fields (`fromMultiplicity`, `toMultiplicity`) and roles (`fromRole`, `toRole`) are only valid on association, aggregation and composition — never on inheritance, realization, or dependency. `meta.viewBox` must keep `viewBox[0] ≤ 1380` so the 1440 px desktop projection leaves the 9 px member text above the 6 px readability floor; default `width` is 180 px. Used for domain models, IHK/UML documentation, hand-off to engineering when inheritance, interfaces and multiplicities must be explicit. Contract and task list: [`renderers/uml-class/README.md`](renderers/uml-class/README.md).
...
Epk note: author events (hexagons), functions (rounded rectangles), connectors (operator circles), organisational units (ellipses with a bar) and information objects (rectangles) on an explicit `meta.grid` (default 250×60 px at `120, 100`). The chain starts with an event without incoming control flow and ends with an event without outgoing control flow, and control flow alternates event → function → event across connectors. Events carry at most one incoming and one outgoing control flow, functions exactly one of each, and a connector is a split (1 in, ≥ 2 out) or a join (≥ 2 in, 1 out). An XOR or OR split may never follow an event — only AND may split after an event. Organisational units and information objects attach to functions only and never take part in the control flow; edge meaning follows the node kinds (solid arrow = control flow, solid line without arrowhead = org assignment, dashed arrow = information flow). Route split connectors with `fromSide: "bottom"` / `toSide: "top"` so branches drop vertically, and keep `meta.viewBox[0] ≤ 1380`. Used for ARIS-style business processes, IHK Geschäftsprozess documentation, and any time responsibilities and information objects belong on the diagram. Contract and task list: [`renderers/epk/README.md`](renderers/epk/README.md).
...
| `activity` | Process flows with responsibilities, parallel steps, Aktivitätsdiagramm with swimlanes for IHK documentation |
...
| `epk` | Business processes, ARIS-style event-driven process chains (eEPK) with responsibilities and information objects for IHK documentation |
...
- `flowchart` / `graph` with decision nodes (`{ }`), I/O (`[/ /]`) or subroutines (`[[ ]]`) → `flowchart`; a plain step graph without decisions → `workflow`; a component map → `architecture`. When a flowchart alternates event and function nodes with gateway diamonds and names responsibilities or information objects, prefer `epk` — keep `flowchart` for algorithms and decision logic. There is no Mermaid equivalent for `struktogramm`: when a user pastes pseudocode or a Nassi-Shneiderman-style nested block sketch, author a fresh `struktogramm` block tree — never try to recover arrows that the source did not have.
- `classDiagram` → `uml-class`; `<|--` inheritance, `*--` composition, `o--` aggregation, `..|>` realization (the source must already mark the `to` as `<<interface>>`), `..>` dependency, and multiplicities written between the dashes (`"1" -- "0..*"`) or as a standalone quoted token. Names, attributes, and operations map to Archify `name`, `attributes`, and `methods` in UML member syntax; drop Mermaid type hints (`~Type~`) and method bodies — Archify carries only the signature.
...
- `flowchart` / `graph` with decision nodes (`{ }`), I/O (`[/ /]`) or subroutines (`[[ ]]`) → `flowchart`; a plain step graph without decisions → `workflow`; a component map → `architecture`. A `flowchart` with `subgraph` blocks acting as lanes (responsibilities) and `{ }` decisions → `activity` when the user wants parallel bars, responsibilities, or a UML activity reading; otherwise → `flowchart`. There is no Mermaid equivalent for `struktogramm`: when a user pastes pseudocode or a Nassi-Shneiderman-style nested block sketch, author a fresh `struktogramm` block tree — never try to recover arrows that the source did not have.
