...
import { validatorExportName } from '../renderers/shared/validator-names.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const root = path.resolve(__dirname, '..');
const schemasDir = path.join(root, 'schemas');
const output = path.join(root, 'renderers/shared/generated-validators.mjs');
const diagramTypes = ['workflow', 'sequence', 'dataflow', 'lifecycle', 'architecture', 'flowchart', 'struktogramm', 'uml-class', 'erd', 'usecase', 'netzplan', 'activity', 'epk'];
...
for (const type of diagramTypes) {
  ...
  schemaIds[validatorExportName(type)] = schema.$id;
}
